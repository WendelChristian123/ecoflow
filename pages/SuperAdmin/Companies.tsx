    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Gerenciamento de Empresas</h1>
                    <p className="text-gray-500 mt-1">Controle detalhado de clientes, planos e módulos</p>
                </div>
            ...
            <Card className="border border-gray-200 shadow-sm">
                <div className="p-4 border-b border-gray-100 flex items-center gap-2">
                    ...
                    <Input
                        ...
                        className="border-none shadow-none focus:ring-0 w-full max-w-md"
                    />
                </div>
                <div className="divide-y divide-gray-100">
                    {filteredCompanies.map(company => (
                        <div key={company.id} className="p-4 hover:bg-gray-50 flex items-center justify-between group transition-colors">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">{(company as any).planName || 'Plano Custom'}</p>
                                </div>

                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button variant="ghost" size="icon" onClick={() => switchCompany(company.id)} title="Acessar como Admin">
                                        <LogIn className="w-4 h-4 text-gray-500 hover:text-indigo-600" />
                                    </Button>
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => handleDelete(company)} title="Excluir">
            ...
            >
                ...
                <div className="flex items-center border-b border-gray-100 mb-6">
                    <button
                        ...
                        className={cn("flex-1 py-3 text-sm font-medium border-b-2 transition-colors", currentStep === 1 ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700")}
                    >
                        1. Dados Básicos
                    </button>
                    <button
                        onClick={() => setCurrentStep(2)}
                        className={cn("flex-1 py-3 text-sm font-medium border-b-2 transition-colors", currentStep === 2 ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700")}
                    >
                        2. Plano e Cobrança
                    </button>
                    <button
                        onClick={() => setCurrentStep(3)}
                        className={cn("flex-1 py-3 text-sm font-medium border-b-2 transition-colors", currentStep === 3 ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700")}
                    >
                                ...
                                {['active', 'suspended', 'trial'].map(s => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            companyStatus === s ? "bg-indigo-50 border-indigo-600 text-indigo-600 ring-1 ring-indigo-600" : "bg-white border-gray-200 text-gray-600 hover:border-gray-300"
                                        )}
                                    >
                                        {s}
                                    </button>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        selectedPlanId === 'custom' ? "border-indigo-600 bg-indigo-50" : "border-gray-100 hover:border-gray-200"
                                    )}
                                >
                                    {selectedPlanId === 'custom' && <div className="absolute top-3 right-3 text-indigo-600"><CheckCircle2 className="w-5 h-5" /></div>}
                                    <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center mb-3">
                                        <Edit className="w-5 h-5 text-gray-600" />
                                    </div>
                                    <h3 className="font-bold text-gray-900">Personalizado</h3>
                                    <p className="text-sm text-gray-500 mt-1">Definir módulos manualmente na próxima etapa.</p>
                                </div>
                                ...
                                {availablePlans.map(plan => (
                                    <div
                                        ...
                                        className={cn(
                                            "cursor-pointer border-2 rounded-xl p-4 transition-all relative",
                                            selectedPlanId === plan.id ? "border-indigo-600 bg-indigo-50" : "border-gray-100 hover:border-gray-200"
                                        )}
                                    >
                                        {selectedPlanId === plan.id && <div className="absolute top-3 right-3 text-indigo-600"><CheckCircle2 className="w-5 h-5" /></div>}
                                        <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mb-3">
                                            <Package className="w-5 h-5 text-blue-600" />
                                        </div>
                                        <h3 className="font-bold text-gray-900">{plan.name}</h3>
                                        <p className="text-lg font-semibold text-gray-900 mt-1">R$ {plan.priceMonthly}<span className="text-sm font-normal text-gray-500">/mês</span></p>
                ...
                {currentStep === 3 && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-200">
                        {/* Categories Tabs */}
                        <div className="flex gap-2 pb-2 border-b border-gray-100 overflow-x-auto">
                            {['Todos', 'Rotinas', 'Financeiro', 'Comercial', 'Relatórios'].map(cat => (
                                <button
                                    key={cat}
                                    onClick={() => setActiveCategoryTab(cat)}
                                    className={cn(
                                        "px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors",
                                        activeCategoryTab === cat ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                    )}
                                >
                                    {cat}
                                </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {SYSTEM_MODULES.filter(m => activeCategoryTab === 'Todos' || activeCategoryTab === 'Rotinas' /* Quick Hack: needs real category mapping */).map(module => (
                                <div key={module.id} className={cn("border rounded-lg p-4 transition-all",
                                    moduleConfig[module.id] === 'included' ? "border-indigo-200 bg-indigo-50" :
                                        moduleConfig[module.id] === 'extra' ? "border-amber-200 bg-amber-50" : "border-gray-200 bg-white"
                                )}>
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            <button
                                                ...
                                                className={cn("p-1.5 rounded-md transition-colors",
                                                    moduleConfig[module.id] !== 'locked' ? "bg-indigo-100 text-indigo-700" : "bg-gray-100 text-gray-400"
                                                )}
                                            >
                                    ...
                                    <p className="text-xs text-gray-500 mb-3 min-h-[40px]">{module.description}</p>
