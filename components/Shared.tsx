...
export const Input: React.FC<InputProps> = ({ className, label, ...props }) => (
  <div className="w-full">
    {label && <label className="block text-xs text-slate-400 mb-1.5 font-medium ml-1">{label}</label>}
    <input 
        className={cn(
        "w-full bg-slate-800 border border-slate-700 text-slate-100 rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none placeholder:text-slate-500 disabled:opacity-50 transition-all",
        className
        )}
        {...props}
    />
