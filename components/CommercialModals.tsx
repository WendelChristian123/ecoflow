...
import { Modal, Input, Button, Select, Textarea, Badge } from './Shared';
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={initialData ? "Editar Item" : "Novo Item de Catálogo"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Input label="Valor Base (R$)" type="number" step="0.01" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} required />
                    <div>
                            ...
                            <tbody className="divide-y divide-slate-800 bg-slate-900/50">
                                {items.map((item, idx) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="p-2">
                                            <input type="number" className="w-full bg-transparent outline-none text-slate-200" value={item.unitPrice} onChange={e => updateItem(idx, 'unitPrice', parseFloat(e.target.value))} />
                                        </td>
                ...
                {step === 2 && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                <Input label="Adicional Recorrente (R$)" type="number" value={extraAmount} onChange={e => setExtraAmount(parseFloat(e.target.value) || 0)} />
                                <Input label="Desconto Mensal (R$)" type="number" value={discount} onChange={e => setDiscount(parseFloat(e.target.value) || 0)} />
                ...
                {step === 3 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 max-w-md mx-auto">
                        ...
                        <div className="pt-2 border-t border-slate-800">
                            <Input label="Taxa de Implantação / Setup (Única)" type="number" value={formData.setupFee} onChange={e => setFormData({...formData, setupFee: parseFloat(e.target.value)})} />
