...
-- Create new function with company_id
...
BEGIN
    ...
    -- 3. Finance
    -- FIX 1: Get Initial Balance from Accounts
    SELECT COALESCE(SUM(initial_balance), 0)
    INTO v_initial_balance
    FROM public.financial_accounts
    WHERE (p_company_id IS NULL OR company_id = p_company_id);

    -- FIX 2: Calculate Transaction Balance (Only Paid) and Other Metrics
    SELECT
        COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE -amount END) FILTER (WHERE is_paid = true), 0),
        COALESCE(SUM(amount) FILTER (WHERE type = 'expense' AND is_paid = false AND credit_card_id IS NULL AND date < v_today), 0),
        COALESCE(SUM(amount) FILTER (WHERE type = 'expense' AND is_paid = false AND credit_card_id IS NULL AND date >= v_today AND date <= v_next_7_date), 0),
        COALESCE(SUM(amount) FILTER (WHERE type = 'income' AND is_paid = false), 0),
        COALESCE(SUM(amount) FILTER (WHERE type = 'income' AND is_paid = false AND date >= v_today AND date <= v_next_7_date), 0),
        COALESCE(SUM(amount) FILTER (WHERE type = 'income' AND is_paid = false AND date < v_today), 0)
    INTO
    ...
    v_balance := v_initial_balance + v_transaction_balance;
