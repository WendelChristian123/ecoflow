-- Migration: update_audit_description
-- Description: Update process_audit_log to add custom description for task creation.
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'tasks' AND TG_OP = 'INSERT' THEN
        IF NEW.assignee_id IS NOT NULL THEN
            ...
            v_description := 'Criação de tarefa para ' || COALESCE(v_assignee_name, 'Usuário') || ': ' || COALESCE(NEW.title, '');
        ELSE
            v_description := 'Criação de tarefa: ' || COALESCE(NEW.title, '');
        END IF;
    END IF;
